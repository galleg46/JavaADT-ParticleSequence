...
public class ParticleSeq implements Cloneable
{
	...
	private boolean _wellFormed() {
		...
		if(_data.length < _manyItems) return _report("data array is shorter than manyItems");
		...
		if(_currentIndex < 0 || _currentIndex > _manyItems) 
			return _report("current index is out of range");
		// 4. if isCurrent is true, then current index must be a valid index
		//    (it cannot be equal to the number of items)
		//if(_isCurrent == true && (_currentIndex < 0 || _currentIndex > _manyItems - 1))
		if(_isCurrent == true && (_currentIndex == _manyItems))
			return _report("there is a current but the current index is not a valid index");
	...
	{
		if(initialCapacity < 0) throw new IllegalArgumentException("Inital Capacity can't be negative");
	...
	public void start( )
	{
		...
		if(_manyItems > 0) _isCurrent = true;
	...
	public Particle getCurrent( )
	{
		...
		if(!isCurrent()) throw new IllegalStateException("there is no current");
	...
	public void advance( )
	{
		...
		if(atEnd()) throw new IllegalStateException("Can't advance, at end");
		
		if(_isCurrent) ++_currentIndex;
		
		_isCurrent = _currentIndex < _manyItems;
	...
	public void addBefore(Particle element)
	{
		...
		int i;
		for(i = _manyItems; i > _currentIndex; --i)
		{
			_data[i] = _data[i - 1];
		}
	...
	public void addAfter(Particle element)
	{
		...
		for(int i = _manyItems - 1; i >_currentIndex; --i)
		{
			_data[i+1] = _data[i];
		}
